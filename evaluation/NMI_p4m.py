...
# 加载特征数据
features = np.load("/gpfs/workdir/islamm/results/se2_features.npy")
# 准备标签数据
#labels = pd.read_csv("/gpfs/workdir/islamm/cluster_assignments.csv").values.squeeze()
encoded_labels = pd.read_csv("/gpfs/workdir/islamm/output_with_labelID (1).csv").values.squeeze()

# ---------------------------
# Load cluster assignments from the CSV file
# ---------------------------

cluster_assignments = pd.read_csv("/gpfs/workdir/islamm/results/p4m_cluster_assignments.csv").values.squeeze()
