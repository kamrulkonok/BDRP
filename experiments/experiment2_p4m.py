...
# Base checkpoint path (we will append epoch number to this)
CHECKPOINT_BASE = os.path.join(RESULT_DIR, "se2_checkpoint")
...
class P4MClusteringCNN(nn.Module):
    def __init__(self, n_features=200):
        ...
        # Input Type
        in_type = e2nn.FieldType(self.p4m_act, [self.p4m_act.trivial_repr])
        ...
        self.fc = nn.Linear(256, n_features)
...
# ---------------------------
# Feature Preprocessing: PCA, Whitening, L2 normalization
# ---------------------------
...
def train(rank, world_size):
    ...
    # Dictionary to keep track of training metrics per epoch
    training_metrics = {
        "epoch": [],
        "nmi": [],
        "average_loss": [],
    ...
    with profile(
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if rank == 0:
                ...
                checkpoint_path = os.path.join(RESULT_DIR, f"se2_checkpoint_epoch{epoch+1}.pth")
                torch.save(checkpoint, checkpoint_path)
                wandb.save(checkpoint_path)
                print(f"Checkpoint saved to {checkpoint_path}")

                # Save training metrics CSV after every epoch
                csv_file = os.path.join(RESULT_DIR, "p4m_training_time_metrics.csv")
