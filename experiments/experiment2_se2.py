...
def save_cluster_assignments(cluster_assignments, output_file=None):
    if output_file is None:
        output_file = os.path.join(RESULT_DIR, "se2_cluster_assignments.csv")
...
def train(rank, world_size):
    ...
    model = SE2ClusteringCNN(n_features=NUM_CLUSTERS).to(device)
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if rank == 0:
                checkpoint = {
                    ...
                    "prev_cluster_assignments": prev_cluster_assignments.cpu().numpy() if prev_cluster_assignments is not None else None,
                }
                torch.save(checkpoint, CHECKPOINT_PATH)
                print(f"Checkpoint saved to {CHECKPOINT_PATH}")

                # Update CSV file after every epoch
                csv_file = os.path.join(RESULT_DIR, "se2_training_time_metrics.csv")
